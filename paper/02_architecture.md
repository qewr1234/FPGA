...
### D. v4: spending multipliers on tap banks

`banked_window_mac` keeps that structure and adds a second axis. Tap *t* is
...
v3. The tuple memory is likewise partitioned. The pipeline gains one stage, the
...
The two axes therefore reach the same multiplier count by different routes: v3
holds *P* multipliers, v4 holds *P* x *T*. A budget of 64 multipliers is v3 with
*P* = 64, or v4 with *P* = 16 and *T* = 4, or several points between. Cycles per
