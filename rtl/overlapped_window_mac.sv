...
module overlapped_window_mac #(
    ...
    // Runtime geometry. K and COUT are now the BUILT-IN MAXIMA -- they size the
    // memories and the counters -- while run_k and run_cout say how much of that
    // this layer actually uses. One bitstream then covers every layer whose
    // weights fit, instead of one bitstream per layer.
    //
    // The weight RAM keeps its compile-time stride (a group starts at group*K,
    // not group*run_k). Addressing by run_k would turn a constant shift into a
    // real multiplier; leaving a gap costs address space that is already built.
